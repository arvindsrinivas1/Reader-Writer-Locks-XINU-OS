...
struct	pentry	{
	...
	int	pwaitret;
	int 	pinh;			/* Inherited priority to solve PI. Initialized with 0 */
	int	locks_held[50];	/* Bitmask */
	//int	lock_id;		/* Waiting lock descriptor */
	int	wait_lock_type;
	int	wait_prio;
	int	plock;			/* initialize to -1 */
	int	lwaitret;
	int	deleted_lock_history[50];
	int	lock_wait_time;
};
