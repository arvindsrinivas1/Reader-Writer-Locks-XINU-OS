...
struct	lentry  rwlocks[NLOCKS];/* locks tabl				*/
...
LOCAL int sysinit()
{
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	pptr->pargs = 0;
	pptr->pprio = 0;
	pptr->wait_lock_type = -1;
	pptr->lock_wait_time = -1;
	//Newly added values to handle rwlocks
	...
	// Initialize rw locks in q
	for(k = 0; k < NLOCKS; k++){
		(lptr = &rwlocks[k])->lstate = LFREE;		
